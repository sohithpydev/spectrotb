...
def find_top_files(data_dir, n=5):
    """Finds top n TB files with STRONG biomarkers (similar to the 'good' example)."""
    ...
    # scan up to 150 files to find good candidates
    print(f"Scanning up to 150 files for high quality spectra...")
    for f in files[:150]:
        mz, inte = load_spectrum(f)
        score = get_biomarker_intensity(mz, inte)
        if score > 500: # Threshold for "good signal"
...
def main():
    print("--- Verifying Baseline Correction on 5 High-Quality Spectra ---")
    current_dir = os.path.dirname(os.path.abspath(__file__))
    pipeline_dir = os.path.dirname(current_dir)
    data_dir = os.path.dirname(pipeline_dir)
    
    top_files = find_top_files(data_dir, n=5)
    ...
    for i, file_path in enumerate(top_files):
        ...
        # Simplified title for clarity
        ax1.set_title(f"File {i+1} - Raw Data", loc='left', fontweight='bold', fontsize=10)
    ...
    out_path = os.path.join(out_dir, 'verify_fix_5files.png')
